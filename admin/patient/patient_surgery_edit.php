...
require_once("../connection.php");
          ...
          $id=$_GET["patient_surgery_id"];
          $patient_sugery=mysqli_query($con, "SELECT * FROM patient_surgery");
          $row_patient_surgery=mysqli_fetch_assoc($patient_sugery);
         ...
         $result= mysqli_query($con,"UPDATE patient_surgery  SET surgery_id=$surgery_id, patient_id=$patient_id, staff_id=$staff_id,date='$date', surgery_result='$surgery_result' where patient_surgery_id=$id");
  
         if($result){
            header("location:patient_surgery_list.php?edit=successfuly");
         }else{
            header("location:patient_surgery_list.php?error=failed");
         }
        }
